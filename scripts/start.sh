...
EOF
	...
	if [ "$MYSQL_DATABASE" != "" ]; then
		...
		echo "GRANT SELECT,INSERT,UPDATE,DELETE ON \`$MYSQL_DATABASE\`.* to '$MYSQL_USER'@'$MYSQL_CLIENT_HOST' IDENTIFIED BY '$MYSQL_PASSWORD';" >> $tfile
		echo "GRANT SELECT,INSERT,UPDATE,DELETE ON \`$MYSQL_DATABASE\`.* to '$MYSQL_USER'@'localhost' IDENTIFIED BY '$MYSQL_PASSWORD';" >> $tfile
	    fi
	fi
...
# Defaults DBs drop
        echo -e "\n\t[i] To drop defaults DBs:"
        echo -e "\t[#] # /scripts/init_db.sh"

# Guacamole DB initialize
	echo -e "\n\t[i] To initialize Guacamole DB:"
	echo -e "\t[#] # cat /scripts/schema/*.sql | mysql -u root -p'$MYSQL_ROOT_PASSWORD' $MYSQL_DATABASE"
